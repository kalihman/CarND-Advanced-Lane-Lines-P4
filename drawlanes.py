...
ret, mtx, dist, rvecs, tvecs  = compute_distortion_coefs()
...
def apply_filters(img, sobel_kernel=3, 
    ...
    # Compute angle formed by absolute value of x and y gradients (and will fall between 0 and pi/2)
    ...
    # plt.subplot(323)
    # plt.imshow((scaled_mag >= thresh_mag[0]) & (scaled_mag <= thresh_mag[1]), cmap='gray')
    # plt.title('Gradient magnitude filter')

    # plt.subplot(324)
    # plt.imshow((angles >= thresh_ang[0]) & (angles <= thresh_ang[1]), cmap='gray')
    # plt.title('Gradient angle filter')

    # plt.subplot(325)
    # plt.imshow((S > thresh_sat[0]) & (S <= thresh_sat[1]), cmap='gray')
...
# This will be passed to video-processing function
def pipeline(img):
    ...
    S, filtered = apply_filters(undist, thresh_lgt=40, thresh_ang=(0.8,1.2), thresh_mag=(40,255), thresh_sat=(100,255))
    ...
    left_fit, right_fit, output = fit_lane_lines(warped)
  
    # Draw the lane line region between the two fitted parabolas back onto the undistorted frame.
    unwarped_lane_region = draw_unwarped_lane_region(warped, undist, Minv, left_fit, right_fit)

    # Below comments were used to create figures for writeup.
    ...
    rl, rr = get_radii_of_curvature(unwarped_lane_region, left_fit, right_fit)
